...
class Chunk():
    ...
    def _parse(self):
        ...
        if self.id == b'MAIN':
            ...
            logging.debug(
                'Detect Pack chunk which is not supportted in current version, skip now')
            ...
            self.voxels = []
            for i in range(n):
                self.voxels.append(unpack_from(
                    XYZI_FMT_2, self.content, 4+4*i))
            ...
            (c_id, r_id, l_id, frames_num) = unpack_from(
                'iiii', self.content, bdict.offset)
            ...
            for i in range(children_num):
                child_ids.append(
                    int(unpack_from('i', self.content, offset+4*i)[0]))
            ...
            for shp in self.nshps:
                if trn.child_node_id == shp.node_id:
                    ...
                    for model in shp.models:
                        trlinks.append((
                            [int(i)
                             for i in trn.frames[frame_index]['_t'].split()],
                            model.id))
        return trlinks
